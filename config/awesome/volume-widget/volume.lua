...
local function GET_VOLUME_CMD(device) return 'pamixer --get-volume' end
local function INC_VOLUME_CMD(device, step) return 'pamixer -i ' .. step end
local function DEC_VOLUME_CMD(device, step) return 'pamixer -d ' .. step end
local function TOG_VOLUME_CMD(device) return 'pamixer -t' end
...
local function worker(user_args)
    ...
    local function update_graphic(widget, stdout)
        ...
        spawn.easy_async("pamixer --get-mute", function(stdout)
            stdout = string.gsub(stdout, "%s+", "")
            if stdout == 'true' then widget:mute()
            elseif stdout == 'false' then widget:unmute()
            end
        end)
        spawn.easy_async("pamixer --get-volume", function(stdout) 
            volume_level = string.format("% 3d", stdout)
